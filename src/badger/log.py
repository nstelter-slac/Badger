...
def set_log_level(level, project_namespace="badger"):
    ...
    for name, logger_obj in logging.root.manager.loggerDict.items():
        if isinstance(logger_obj, logging.Logger) and name.startswith(
            project_namespace
        ):
            logger.info(
                f"Setting logger {logger_obj.name} to level {logging.getLevelName(level)}"
            )
            logger_obj.setLevel(level)
...
class CentralizedLoggingManager:
    ...
    def start_listener(self, log_filepath: str, level, gui_callback=None):
        ...
        logger.info(
            f"Centralized logging listener started with level {logging.getLevelName(level)}"
        )
    ...
    def update_log_level(self, level):
        ...
        logger.info(
            f"Log level updated to {logging.getLevelName(level)} for all handlers"
        )
