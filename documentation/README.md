...
Any new additions or edits to the documentation should be made here in the Badger repo's `/documentation` folder.
The homepage source must build successfully on PRs before they can be merged into main.
Then once merged the main branch will trigger an automatic deployment of the homepage website.

### Building Homepage Locally

After making changes to the Badger Homepage, you can test things out by building it locally:

1. install nodejs npm (on linux this is probably in the package manager, for example on ubuntu try: `sudo apt install -y nodejs npm`)
2. `cd Badger/documentation`
3. `npm install`
4. `npm run build` (if this fails there are issues with your changes!)
5. `npm run serve`
5. now open an internet-broweser to address `http://localhost:3000` to view the homepage!
